    ...
    for directory in directories:
        ...
        for fname in os.listdir(directory):
            if fname.endswith(".json"):
                config_name = "{dname}-{fname}".format(dname=directory, fname=".".join(fname.split('.')[:-1]))
...
def create_recreate_index(es, index_name, aliases, fields, delete_index=False, exists_ok=True):
    if delete_index:
        try:
...
def get_source(config, source_type='mysql'):
    from es_data_load.DataSources import TabularDataSource, MySQLDataSource, DelimitedDataSource
