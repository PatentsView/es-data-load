...
AVAILABLE_SCHEMA_FILES = {'granted': {'assignees_fields.json': 'assignees{suffix}',
                                      ...
                                      'wipo_fields.json': 'wipo{suffix}'}, 'pregrant': {}}
...
class PVSchemaManager(SchemaManager):
    ...
    def load_default_schema(cls, schema_files, suffix, schema_path_prefix):
        ...
        for file_name in schema_files:
            ...
            with pkg_resources.path(f"es_data_load.pv.schemas.{schema_path_prefix}", file_name) as schema_file:
                field_mapping = json.load(open(schema_file, 'r'))
            index_name = index_pattern.format(suffix=suffix)
            schemas[f"{schema_path_prefix}_{file_name}"] = {"index_name": index_name,
                                                            "field_mapping": field_mapping, 'type': schema_path_prefix,
                                                            'index_pattern': index_pattern}
        return schemas

    @classmethod
    def load_default_pv_schema(cls, es, suffix, granted_schema_files=None, pregrant_schema_files=None):
        ...
        selected_schemas = cls.load_default_schema(granted_schema_files, suffix, 'granted')
        selected_schemas.update(cls.load_default_schema(pregrant_schema_files, suffix, 'pregrant'))
        return cls(es=es, schemas=selected_schemas, suffix=suffix)
