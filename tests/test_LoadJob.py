...
def test_process_load_operation(pv_citations_only_schema_manager, load_job_configuration_1: LoadConfiguration, search,
        mysql_source, ):
    load_job = LoadJob(load_configuration=load_job_configuration_1, data_source=mysql_source, data_target=search,
                       test=False)
    name_load_job_to_process = load_job_configuration_1.get_load_operation_names()[0]

    pv_citations_only_schema_manager.create_es_indices(drop_and_recreate=True, exists_ok=True)

    load_job.process_load_operation(name_load_job_to_process)
    assert_es_counts(search=
                     load_job.search_target,
                     target_setting=load_job.load_configuration.get_load_operation(name_load_job_to_process)[
                         'target_setting'])
...
def test_get_load_job_status(load_job: LoadJob):
    ...
    try:
        load_job.search_target.es.indices.delete(
            index=load_job.load_configuration.get_load_operation(name_load_job_to_process)['target_setting']['index'])
...
def test_process_csv_load_operation(pv_citations_only_schema_manager, load_job_configuration_csv: LoadConfiguration,
        search, tabular_source):
    load_job = LoadJob(load_configuration=load_job_configuration_csv, data_source=tabular_source, data_target=search,
                       test=False)
    name_load_job_to_process = load_job_configuration_csv.get_load_operation_names()[0]
    pv_citations_only_schema_manager.create_es_indices(exists_ok=True, drop_and_recreate=True)
    load_job.process_load_operation(name_load_job_to_process)
    target_setting = load_job.load_configuration.get_load_operation(name_load_job_to_process)[
        'target_setting']
