...
from es_data_load.pv.schemas.granted.PVSchemaManager import AVAILABLE_SCHEMA_FILES, PVSchemaManager
...
def test_schema_mapping_crosswalk():
    assert len(AVAILABLE_SCHEMA_FILES['granted']) == len(AVAILABLE_MAPPING_FILES['granted'])
    assert len(AVAILABLE_SCHEMA_FILES['pregrant']) == len(AVAILABLE_MAPPING_FILES['pregrant'])


def test_pv_schema_load(search):
    sm = PVSchemaManager.load_default_pv_schema(suffix="_test",
                                                es=search.es)
    assert len(sm.schemas) == len(AVAILABLE_SCHEMA_FILES['granted']) + len(AVAILABLE_SCHEMA_FILES['pregrant'])
    ...
    sm = PVSchemaManager.load_default_pv_schema(suffix="_test",
                                                es=search.es,
                                                granted_schema_files=random_choices)
...
def test_go_live(search):
    ...
    for file_name in random_choices:
        ...
        assert sm.schemas[f"granted_{file_name}"]["field_mapping"] == mapping_from_es[index_name]["mappings"][
            "properties"]
