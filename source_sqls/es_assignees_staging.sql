...
insert into elastic_staging.assignees( assignee_id, type, name_first, name_last, organization, num_patents
...
select distinct
  ...
  , persistent_assignee_id
from
    PatentsView_20211230.assignee a
        join PatentsView_20211230.patent_assignee pa
             on pa.assignee_id = a.assignee_id
        join elastic_staging.patents p on p.patent_id = pa.patent_id
        left join PatentsView_20211230.temp_id_mapping_location timl on timl.new_location_id = a.lastknown_location_id;



create table elastic_staging.assignee_years like PatentsView_20211230.assignee_year;

insert into elastic_staging.assignee_years(assignee_id, patent_year, num_patents)
select
    ay.assignee_id
  , patent_year
  , ay.num_patents
from
    PatentsView_20211230.assignee_year ay
        join elastic_staging.assignees a
where
